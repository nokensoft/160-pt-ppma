        ...
        <div class="container mx-auto px-4">
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-12">
                <div class="lg:col-span-3">
                    <div class="mb-6">
                        @if ($berita->kategori)
                            <span class="text-lg bg-primary text-white px-3 py-1 font-bold uppercase">{{ $berita->kategori->nama }}</span>
                        @endif
                    </div>
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-200" x-data="{ copied: false }">
                        ...
                        <div class="flex flex-wrap gap-3">
                            <button @click="navigator.clipboard.writeText('{{ route('berita.detail', $berita->slug) }}'); copied = true; setTimeout(() => copied = false, 2000)"
                                    class="flex items-center gap-2 px-4 py-2.5 border border-gray-300 text-gray-600 hover:border-primary hover:text-primary transition text-lg font-semibold">
                ...
                <div>
                    <div class="bg-gray-50 p-6 mb-6 rounded-lg">
                        <h4 class="font-bold uppercase text-lg mb-4 pb-3 border-b-2 border-primary">Cari Berita</h4>
                        <form method="GET" action="{{ route('berita') }}">
                            <div class="relative">
                                <input type="text" name="cari" placeholder="Cari berita..." class="w-full border border-gray-300 p-3 pr-12 text-lg focus:border-primary focus:outline-none transition no-round">
                    ...
                    <div class="bg-gray-50 p-6 mb-6 rounded-lg">
                        <h4 class="font-bold uppercase text-lg mb-4 pb-3 border-b-2 border-primary">Berita Lainnya</h4>
                    ...
                    <a href="{{ route('berita') }}" class="bg-dark text-white p-5 block hover:bg-primary transition text-center">
                        ...
                        <span class="font-bold text-lg uppercase">Semua Berita</span>
                    </a>
                </div>
            </div>
        </div>
